...
values("Shreyasahana","shreyasahana",123);
...
values("Shreya","shreyausadalgekar@gmail.com","8951142969");
...
SELECT 
    blood.blood_group,
    COUNT(donor_details.donor_id) * 400 AS total_volume
FROM 
    donor_details
JOIN 
    blood ON donor_details.donor_blood = blood.blood_id
GROUP BY 
    blood.blood_group;


DELIMITER //
CREATE PROCEDURE update_inventory(IN in_blood_id INT, IN volume_to_add INT)
BEGIN
    UPDATE blood_inventory
    SET total_volume = total_volume + volume_to_add
    WHERE blood_group = (SELECT blood_group FROM blood WHERE blood_id = in_blood_id);
END//
...
FOR EACH ROW
BEGIN
    CALL update_inventory(NEW.donor_blood, 400);
END//

CREATE FUNCTION check_low_inventory(blood_type VARCHAR(5)) RETURNS BOOLEAN
BEGIN
    DECLARE total_volume INT;
    SELECT total_volume INTO total_volume FROM blood_inventory WHERE blood_group = blood_type;
...
ALTER TABLE blood 
ADD COLUMN total_volume INT DEFAULT 0;
SELECT * FROM blood_inventory;

ALTER TABLE donor_details MODIFY donor_blood INT;

UPDATE admin_info 
SET admin_username = 'ShreyaSahana', 
    admin_password = SHA2('123',256), 
    admin_name='ShreyaSahana'
WHERE admin_id = 1;
...
ADD COLUMN donor_medical_history TEXT;
...
AFTER DELETE ON donor_details
FOR EACH ROW
BEGIN
    INSERT INTO donor_data (donor_id, donor_name, donor_number, donor_mail, donor_age, donor_gender, donor_blood, donor_address, status)
    VALUES (OLD.donor_id, OLD.donor_name, OLD.donor_number, OLD.donor_mail, OLD.donor_age, OLD.donor_gender, OLD.donor_blood, OLD.donor_address, 'Not Donated');
END;
//
DELIMITER ;
...
BEGIN
    INSERT INTO donor_data (donor_id, donor_name, donor_number, donor_mail, donor_age, donor_gender, donor_blood, donor_address, status)
    VALUES (OLD.donor_id, OLD.donor_name, OLD.donor_number, OLD.donor_mail, OLD.donor_age, OLD.donor_gender, OLD.donor_blood, OLD.donor_address, 'Donated the Blood');
END;
//
DELIMITER ;
